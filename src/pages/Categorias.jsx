    ...
    <div style={styles.layout}>
      ...
      <main style={styles.main}>
        ...
        <div style={styles.pastas}>
          {categorias.map(cat => (
            <div key={cat.id} style={styles.pasta}>
              <div style={styles.pastaHeader} onClick={() => setCategoriaAberta(categoriaAberta === cat.id ? null : cat.id)}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span>{categoriaAberta === cat.id ? '▲' : '▼'}</span>
                </div>
              </div>
              ...
              {categoriaAberta === cat.id && (
                <div style={styles.pastaConteudo}>
                  {projetosDaCategoria(cat.id).length === 0 ? (
                    <p style={styles.vazio}>Nenhum projeto nesta pasta ainda.</p>
                  ) : (
                    projetosDaCategoria(cat.id).map(p => (
                      <div key={p.id} style={styles.projetoItem} onClick={() => navigate(`/projetos/${p.id}`)}>
                        <div style={{ flex: 1 }}>
        ...
        {projetosSemCategoria.length > 0 && (
          <div style={{ marginTop: 24 }}>
            <h2 style={styles.sectionTitulo}>📋 Projetos sem pasta</h2>
...
const styles = {
  ...
  pasta: { background: '#fff', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' },
  ...
  pastaCurso: { color: '#C8102E', fontSize: 13, fontWeight: 600, margin: '2px 0 0' },
  ...
  btnDeletar: { background: '#FEE2E2', color: '#C8102E', border: 'none', borderRadius: 6, padding: '6px 10px', cursor: 'pointer', fontSize: 14 },
  vazio: { color: '#999', fontSize: 13, textAlign: 'center', padding: '20px' },
  sectionTitulo: { fontSize: 18, fontWeight: 700, color: '#1A1A1A', marginBottom: 12 },
};